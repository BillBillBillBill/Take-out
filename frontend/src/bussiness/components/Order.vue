    ...
    <div class="row small-up-1 medium-up-2 large-up-3">
      <template v-for="order in orders">
        <div class="column order-info">
          <div class="row">订单日期：{{order.order_date}}</div>
          <div class="row">收货人姓名：{{order.customer_name}}</div>
          <div class="row">收货人地址：{{order.customer_address}}</div>
          <div class="row">收货人联系电话：{{order.customer_tel}}</div>
          <div class="row">订单食物数量：{{order.order_amount}}件</div>
          <div class="row">订单总价：<i class="fi-yen"></i> {{order.order_total}}</div>
          <div class="row">订单状态：{{order.order_status}}</div>
          <div class="row">
            <template v-if="order.order_status == '未处理'">
              <div class="button receive-reject">接单</div>
              <div class="button receive-reject">拒单</div>
            </template>
            <div v-if="order.order_status == '已接单'" class="">配送中</div>
          </div>
        </div>
      </template>
    </div>
